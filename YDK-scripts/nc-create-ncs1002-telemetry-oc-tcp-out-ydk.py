...
"""
Create configuration for Telemetry TCP Dial-out mode using
openconfig-telemetry model.
usage: nc-create-ncs1002-telemetry-oc-tcp-out-ydk.py [-h] [-v]\
...
def config_telemetry_system(telemetry_system):
    ...
    for i in [[S1_PORT, SERVER_1],
        ...
        if i[1] is not None:    
            destination_group = telemetry_system.destination_groups.\
                                DestinationGroup()
            ## the name of this destination group
            destination_group.group_id = DGROUP_ID
            destination_groups = destination_group.Config()
            destination_groups.group_id = DGROUP_ID
            destination_group.config = destination_groups
            ## address and port of the server configuration
            destination = destination_group.destinations.Destination()
            destination.destination_port = i[0]
            destination.destination_address = i[1]
            destinations = destination.Config()
            destinations.destination_port = i[0]
            destinations.destination_address = i[1]
            ## define the transport protocol 
            destinations.destination_protocol = oc_telemetry.\
                                                TelemetryStreamProtocolEnum.TCP
            destination.config = destinations
            destination_group.destinations.destination.append(destination)
            telemetry_system.destination_groups.\
    ...
    for i in [[SUBS1_ID, SGROUP1_ID, SGROUP1_INT],
        ...
        destination_profile = subscription.destination_groups.DestinationGroup()
        destination_profile.group_id = DGROUP_ID
        destination_profiles = destination_profile.Config()
        destination_profiles.group_id = DGROUP_ID
        destination_profile.config = destination_profiles
        subscription.destination_groups.destination_group.append(destination_profile)
